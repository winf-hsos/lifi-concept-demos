...
**The Copier** (Analog und digital, live): eine Kopienwand. Jeder Druck auf "copy" haengt die naechste analoge Generation daneben (die Bilder schrumpfen, damit alles passt), Kopiersorgfalt in drei Worten statt Sigma; der Kontrastknopf "and as a file?" haengt die identische Dateikopie dazu. Ohne Obergrenze; drei Fotomotive (KI-generiert), geteilt mit dem Photo Digitiser. Umgebaut am 03.09.2026 von der Zwei-Ketten-Fassung.

**The Photo Digitiser** (Analog und digital, live): analoges Foto (drei Motive, geteilt mit dem Copier) gegen seine Digitalisierung; Regler fuer Aufloesung (256x256 bis hinunter zu 1 Pixel) und Farbtiefe (1 bit s/w, 8 bit grau, 8/16/24 bit Farbe), darunter die offene Rechnung Pixel x Bits = Dateigroesse plus Uebertragungsdauer bei 30 bit/s ueber die Lichtstrecke. Seit dem 14.09.2026 zeigt das Ueberfahren eines Pixels dessen Wert im gewaehlten System (Bits nach Kanaelen gruppiert, daneben die Zahlen) und umrandet die Zelle auf beiden Bildern.

**The Audio Digitiser** (Analog und digital, live): dieselben zwei Schnitte fuers Ohr. Fuenf Klaenge (Ode an die Freude, kleines Bandstueck, Bassfigur und Vogelzwitschern prozedural, dazu eine KI-generierte Sprachaufnahme), Abtastrate 48 kHz bis 1 kHz und Bit-Tiefe 16/12/8/4/2/1 bit; Treppenkurve ueber der glatten Welle, Abspielknoepfe fuer Original und Digitalfassung (Web Audio, keine Abhaengigkeit), Dateigroesse und Lichtstrecken-Dauer live. Bewusst naive Digitalisierung ohne Filter, ehrlich ausgewiesen.
...
**The Prefix Trap** (Codesysteme, live): der Code, der nicht funktioniert, zum Selbermachen, angeordnet als Strecke (Sender links mit seinen Codewoertern und dem Wort, in der Mitte die nackte Bitfolge auf der Leitung, rechts der Empfaenger). Vier eigene Codewoerter fuer a bis d, ein getipptes Wort, und beim Empfaenger **alle** Zerlegungen, zwischen denen er waehlen muesste. Vorbelegung a=0, b=01, c=10, d=11 mit dem Wort "bad": zwei Lesarten. Der Befund nennt, welches Codewort welches beginnt, sagt aber nicht, wie es richtig geht; wer feste Laenge oder Praefixfreiheit selbst herstellt, sieht die Liste auf eins schrumpfen. Startwerte in der Adresse (?a=&b=&c=&d=&word=).
...
### Verarbeitung: „the pixel filter" (gebaut am 07.09.2026, `pixel-filter/`)

Ein 128×128-Graustufenbild (Papagei, Sonnenuntergang, Leuchtturm) mit Filterknöpfen: `brighter +40`, `darker −40`, `invert`, `black & white` (Schwelle verschiebbar), `blend` mit dem zweiten Motiv; die Rechenzeile (`new = old + 40`) steht über dem Bild. Zwei Betriebsarten: `step` bearbeitet ein Pixel und zeigt es gelb markiert mit seiner Zahl, den acht Bits, der binären Addition samt Überträgen (bei invert das Bitkippen, bei s/w den Vergleich), dem Ergebnis und dem Zähler `additions: 1`; `run` macht den Rest in zwei Sekunden zeilenweise wie ein Scanner. Darunter die Größenordnung `16 384 additions · about 650 000 gate switches` (8 Volladdierer je Addition, etwa 5 Gatter je Volladdierer). Schalter `at 255: clamp / wrap`: bei wrap zeigt brighter schwarze Sprenkel in hellen Flächen, die Schrittansicht den neunten Übertrag rot durchgestrichen. Nicht enthalten: Farbbilder, Faltungsfilter (Fußnote „same idea, nine pixels at a time"), Schaltpläne. **Didaktischer Kern:** Verarbeitung ist dieselbe stupide Rechnung, sehr oft; deckt pc-009 bis pc-011 und macht pc-003/pc-007 anschaulich. Deck 12, Folie 22 und die Konzeptseite verlinken bereits auf `pixel-filter/`. **Gebaut wie beschrieben**, dazu ein Schalter „1 · greyscale / 3 · colour“ (in Farbe drei Rechnungen je Pixel in drei Spalten, 49.152 Additionen, bunte Sprenkel bei wrap; Schwarz-Weiß rechnet erst die Helligkeit), mit fünf Filtern, step/run, clamp/wrap, offener Rechnung je Pixel (Bits, Überträge, neunter Übertrag rot durchgestrichen) und Gatterzähler; Tastatur: s, r, 0, 1 bis 5.

### Logik und Arithmetik: „the gate lab" (gebaut am 08.09.2026, `gate-lab/`)

Die einfachen Schaltkreise aus Deck 12 zum Anfassen, Schalter am Eingang, Lämpchen am Ausgang: die vier Gatter, Halbaddierer, Volladdierer, Byte-Addierer aus acht Volladdierern mit sichtbar laufendem Übertrag, Flip-Flop (setzen, halten, löschen), Register. Stationen einer Demo, einzeln aufrufbar per Parameter (`?circuit=full-adder`) und mit Werten vorbelegbar (`&a=178&b=40`). **Verknüpfung:** Der Pixel Filter verlinkt aus seinem Rechenfeld die gerade arbeitende Schaltung mit den aktuellen Bytes: Addition → Byte-Addierer, invert → acht Nicht-Gatter, black & white → Vergleicher, clamp → Vergleich vor dem Speichern. Dieselbe Rechnung als Zahlen, als Bits, als Gatter. **Gebaut:** acht Stationen (gates, half adder, full adder, byte adder mit aufklappbaren Blöcken, compare als a + (255 − b) + 1, flip-flop aus zwei NOR, d latch als Türsteher davor, register aus acht d latches mit store als Pegel). Eine Baustation mit Puzzles (Gatter per Drag & Drop in leere Plätze) war kurz drin und ist auf Nutzerwunsch wieder entfernt; freies Verdrahten wäre ohnehin ein Schaltungseditor. Schalter sitzen als Kippschalter direkt in der Zeichnung, Gatter tragen keine Beschriftung. Netzlisten-Simulation mit Iteration bis zur Ruhe, Zeichnung als SVG, Eingaben als echte Buttons. Der Pixel Filter verlinkt aus dem Rechenfeld hierher, mit Werten vorbelegt.

### Verarbeitung: „the substrate independence lab" (Idee, 08.09.2026)

Ein Gatter in vier Bauweisen nebeneinander, animiert, mit Schaltern und Lampe: Relais mit Feder, Vakuumröhre, Transistor, Neuron (Erregung, Schwelle, Aktionspotenzial; erregend und hemmend ergeben Und, Oder, Nicht). Dieselbe Wahrheitstafel unter allen vieren, je Bauweise Schaltzeit und Baujahr. **Didaktischer Kern:** Logik hängt nicht am Silizium; alles mit einer Schwelle, das von etwas anderem gesteuert wird, kann ein Gatter sein. Verlinkung aus dem Gate Lab. **Aufwand: mittel bis hoch (Animationen). Priorität: mittel.**

### Speichern: „the memory lab" (Idee, 08.09.2026)

Ein Bit in verschiedenen Speichermechanismen nebeneinander, je mit write 1, write 0, read: Rückkopplungsschleife (SRAM, das Flip-Flop aus dem Gate Lab), leckender Kondensator mit Refresh (DRAM), Magnetisierung (Kernspeicher, Festplatte), gefangene Ladung (Flash), Vertiefung in einer Oberfläche (CD, Lochkarte, Braille), zum Kontrast das Kurzzeitgedächtnis. Je Mechanismus: flüchtig oder nicht, Schreib- und Lesezeit, Kosten je Bit als Größenordnung. **Didaktischer Kern:** Ein Bit ist ein Zustand, der bleibt; wie er bleibt, ist austauschbar, und die Unterschiede erklären die Speicherhierarchie. Konzept Memory. **Aufwand: mittel bis hoch. Priorität: mittel.**
...
### Kompression: „The Text Squeezer" (gebaut am 14.09.2026, `text-squeezer/`)

Ein Textfeld; beim Tippen wird mit den zwei Tricks aus Deck 15 gepackt: Zurückzeigen (Lempel-Ziv, Verweis als Marke plus 12 Bit Abstand plus 8 Bit Länge, nur wo er sich lohnt) und Zählen (Huffman über die verbleibenden Zeichen samt Marke, Tabelle je möglichem Zeichen 5 Bit). Drei Balken gegen den kleinsten festen Code des Textalphabets: nach Trick eins, nach beiden. Sechs Beispiele: ein Satz (zu kurz, wächst), ein Buchstabe 400-mal (13 %), ein Absatz (89 %), ein Logfile (44 %), zufällige Buchstaben aus genau 32 Zeichen (107 %, wächst), ein Bitmuster. Darunter die Verweise im Text und die Codetabelle. **Didaktischer Kern:** Was wiederkehrt oder häufig ist, lässt sich kürzer sagen; was jedes Mal neu ist, nicht, und die Buchführung macht es größer. Ersetzt die frühere Idee „Nachricht schrumpfen" (nur Huffman). Startparameter `?preset=<name>`.

Frühere Fassung der Idee: „Nachricht schrumpfen"
