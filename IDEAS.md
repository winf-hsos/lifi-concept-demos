...
Die einfachen Schaltkreise aus Deck 12 zum Anfassen, Schalter am Eingang, Lämpchen am Ausgang: die vier Gatter, Halbaddierer, Volladdierer, Byte-Addierer aus acht Volladdierern mit sichtbar laufendem Übertrag, Flip-Flop (setzen, halten, löschen), Register. Stationen einer Demo, einzeln aufrufbar per Parameter (`?circuit=full-adder`) und mit Werten vorbelegbar (`&a=178&b=40`). **Verknüpfung:** Der Pixel Filter verlinkt aus seinem Rechenfeld die gerade arbeitende Schaltung mit den aktuellen Bytes: Addition → Byte-Addierer, invert → acht Nicht-Gatter, black & white → Vergleicher, clamp → Vergleich vor dem Speichern. Dieselbe Rechnung als Zahlen, als Bits, als Gatter. **Gebaut:** acht Stationen (gates, half adder, full adder, byte adder mit aufklappbaren Blöcken, compare als a + (255 − b) + 1, flip-flop aus zwei NOR, register, build it yourself mit den Puzzles Halb- und Volladdierer per Drag & Drop oder Klick, Wahrheitstafel als Prüfung). Freies Verdrahten bewusst nicht: das wäre ein Schaltungseditor. Netzlisten-Simulation mit Iteration bis zur Ruhe, Zeichnung als SVG, Eingaben als echte Buttons. Der Pixel Filter verlinkt aus dem Rechenfeld hierher, mit Werten vorbelegt.
